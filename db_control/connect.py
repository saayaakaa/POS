...
try:
    ...
    if (ENVIRONMENT == 'production' or 
        ...
        # Azure Database for MySQL用のSSL設定（PyMySQL 1.4.6対応）
        connect_args = {
            'charset': 'utf8mb4'
        }
        
        if 'azure.com' in DB_HOST or DB_SSL_MODE == 'REQUIRED':
            # Azure Database for MySQL用のSSL設定（最新PyMySQL対応）
            ssl_cert_path = os.path.join(os.path.dirname(__file__), 'DigiCertGlobalRootG2.crt.pem')
            
            # PyMySQL 1.4.6用のSSL設定
            ssl_config = {}
